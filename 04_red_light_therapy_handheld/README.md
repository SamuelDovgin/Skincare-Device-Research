...
## Contents
- The topic viewer includes an [FDA-reported events panel](index.html#maude) for the OHS light-based wrinkle-reduction lane. It is a reported-event signal, not an incidence rate or causal safety ranking.
- **[Localized PBM evidence and N-of-1 protocol](index.html#doc7)** — the missing hands/carpal-tunnel/pain lane: conflicting meta-analyses, sham-controlled nulls, a direct home-use trial, panel-to-probe translation limits, and a measurement-led personal audit.
...
- **[Whole-body dose-response visualizer](whole_body_dose_response_visualizer.html)** — interactive graph showing the central illustrative benefit peak around 15–20 J/cm² and the modeled decline after the peak, with human protocol locations marked separately from the curve; includes an alternating-area cadence calculator for daily panel use without treating the same defined area every day.
- **[Localized PBM N-of-1 log](data/localized_pbm_n_of_1_log.csv)** — CSV template for a seven-day baseline, fixed 21-day exposure phase, and seven-day observation phase without endpoint switching.
