...
One correction to a too-simple version of this argument: **"the follicle peak mostly resets" does not mean "all heat disappears," and thermal reset does not erase all pulse history.** Published TRT is a **50%-heat-loss time**, not a full-baseline time. At a 250 ms gap, the single-exponential residual is about 18% for a 100 ms half-time and about 1% for a 40 ms half-time; deeper tissue can retain more. More surprisingly, direct human-hair experiments found that later alexandrite/Nd:YAG pulses produced larger rises even when ≥2 s spacing plus an air blower let measured temperature return to ambient. That separate hair-response memory lasted about 5 s around 55 °C and >60 s after >70 °C exposure.[[21]](thermal_model_source_docs/PMC10107531_hair_temperature_avalanche_fulltext.xml) The simulator now exposes both mechanisms separately and labels the response-memory layer as a cross-wavelength IPL extrapolation.
...
- **Heat-pain / sensory load:** human heat-pain thresholds sit around **42–43°C**, TRPV1 is linked to noxious heat detection, and modern heat-pain imaging shows the neural response can rise late during a sustained heat stimulus rather than peaking only at the instant of first contact. [[18]](https://pubmed.ncbi.nlm.nih.gov/11872608/)[[19]](https://www.nature.com/articles/s41598-025-99247-7)[[20]](https://pubmed.ncbi.nlm.nih.gov/36454632/) This is why "last pulse stings most" is physiologically plausible even when the follicle peak itself mostly resets.
...
Here's the thermal mechanism in plain terms. If a second flash arrives **before** the follicle has cooled back down from the first, it isn't starting from zero — it lands on residual heat and the combined temperature climbs. If it arrives after most temperature excess has dissipated, the direct thermal peak stacks much less. But the 2023 hair data add a second possibility: even a pulse that starts thermally near baseline may heat hair more strongly because prior heating temporarily changed its response. The clinical literature describes the repeated-heating strategy as **progressive photothermolysis**:
...
This illustrative model (not measured device data) now treats the stated TRT correctly as a **half-time**. An IPL-specific Monte Carlo/heat-diffusion paper defines TRT as the time to lose 50% of induced heat, places 200–300 µm follicles around 40–100 ms, and found that close pulse stacks retained target heating more efficiently than stacks with longer off-times.[[22]](thermal_model_source_docs/PMC5718238_broadband_IPL_pulse_structure_model_fulltext.xml) Fired ~20 ms apart, later pulses land on substantial residual heat. Fired ~200–250 ms apart, heater-peak stacking is weaker but not mathematically zero for coarse follicles; deep heat and the optional hair-response-memory state can still accumulate. The latter is shown as a sensitivity scenario because it was measured with hair-removal lasers, not a broadband home IPL burst.

So the concrete answer to "how quick do the flashes need to be": for strong **residual-temperature peak stacking** across 2–3 discrete flashes, the gap should be no longer than roughly one follicle TRT half-time—practically under about 40–100 ms for terminal follicles, with 10–40 ms retaining much more heat. At 200–250 ms, direct heater carryover ranges from small to modest depending on diameter; it should not be described as fully reset. A separate pulse-history effect may persist for seconds, but its magnitude for home IPL is unknown.
...
18. Defrin R, Ohry A, Blumen N, Urca G. Sensory determinants of thermal pain. *Brain*. 2002;125(Pt 3):501-510. https://pubmed.ncbi.nlm.nih.gov/11872608/ — normal heat-pain thresholds around 42°C.
19. Nold JI, Tinnermann A, Fadai T, Mintah M, Morgenroth MS, Büchel C. Comparing neural responses to cutaneous heat and pressure pain in healthy participants. *Scientific Reports*. 2025;15:14387. https://www.nature.com/articles/s41598-025-99247-7 — heat-pain responses can rise toward the late phase of a sustained stimulus.
20. Katz B, Zaguri R, Edvardson S, et al. Nociception and pain in humans lacking a functional TRPV1 channel. *J Clin Invest*. 2023. https://pubmed.ncbi.nlm.nih.gov/36454632/ — human evidence linking TRPV1 function to noxious heat pain.
21. Viera-Mármol G, et al. *Measurements of hair temperature avalanche effect with alexandrite and Nd:YAG hair removal lasers.* Lasers Surg Med. 2023;55:89-98. https://pmc.ncbi.nlm.nih.gov/articles/PMC10107531/ — temperature returned to ambient between ≥2 s-spaced pulses, yet later pulses produced nonlinear temperature gain; response lifetime about 5 s near 55 °C and >60 s above 70 °C; [local full text](thermal_model_source_docs/PMC10107531_hair_temperature_avalanche_fulltext.xml).
22. Ash C, et al. *Mathematical modeling of the optimum pulse structure for safe and effective photo epilation using broadband pulsed light.* J Appl Clin Med Phys. 2012;13:290-299. https://pmc.ncbi.nlm.nih.gov/articles/PMC5718238/ — IPL-specific Monte Carlo/heat-diffusion model; defines TRT as 50% heat loss and compares close versus spaced pulse stacks; [local full text](thermal_model_source_docs/PMC5718238_broadband_IPL_pulse_structure_model_fulltext.xml).

*Compiled 2026-06-30; updated 2026-07-10 to separate residual temperature, deeper heat, and pulse-history sensitivity and to correct TRT half-time math. This document's numbered citations are peer-reviewed/StatPearls sources on the general TRT/TDT/SHR physics; the device-specific figures (T023A measurements, FDA pulse-energy data) are drawn from this repo's own prior research — see the inline local links throughout rather than the numbered list for those.*
