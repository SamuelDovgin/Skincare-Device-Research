...
One correction to a too-simple version of this argument: **"the follicle peak mostly resets" does not mean "all heat disappears."** At a 250ms gap, a coarse follicle may have only a small residual peak tail left, but deeper tissue and local heat-pain/sting signaling can carry forward on a slower hundreds-of-milliseconds clock. That is a very plausible reason the **last pulse can sting the most**. It does **not** by itself prove the later pulse is killing the follicle better; it proves the body is sensing cumulative heat/sting load, which is related but not identical to follicle peak temperature. The updated [SHR thermal simulator](shr_thermal_simulator.html) now draws this separately as a purple sting-proxy curve.
...
This illustrative model (not measured device data — the shape is what the physics predicts, using a ~70ms cooling half-life from the middle of the follicle-TRT literature above) makes the timing requirement concrete: **the same total energy, split into the same three equal pulses, either crosses a follicle-peak damage threshold or never gets close — purely depending on whether the gap between flashes is shorter or longer than the follicle's own cooling curve.** Fired ~20ms apart (well inside the ~10–100ms follicle TRT), the third pulse lands on enough residual follicle heat from the first two to push the peak temperature past where any single one of them could reach. Fired ~200–250ms apart (well outside most of it), each pulse individually cools most of the way back down before the next arrives, and the **follicle peak** never gets meaningfully higher than one pulse alone. **But slower sensory/deep heat can still accumulate**, which is why a pulse train may feel sharper at the end without behaving like one big follicle dose. Nothing about the total energy changed between those scenarios — only the clock did.

So the concrete answer to "how quick do the flashes need to be": for genuine **follicle-peak** accumulation across 2–3 discrete flashes at a fixed spot, the gap needs to sit meaningfully **under the follicle's own TRT — practically, under 50–100ms**, and ideally down in the 10–40ms range that clinical fixed-spot multi-pulse protocols already use and have validated (next section). Push much past ~100–200ms and you're no longer strongly stacking the follicle peak — you're mostly re-dosing the same weak shot, while still accumulating some slower deep/sensory heat.
