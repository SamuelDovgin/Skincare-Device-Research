...
**Temperature reality-check (spot-checked against the literature).** The simulator's three thresholds hold up: **follicle coagulation ≈65 °C** matches the clinical target (65–70 °C; 60 °C minimum; time-dependent 70 °C@1 ms / 62 °C@100 ms); the **sustained-damage floor ≈45 °C** matches SHR's stated 45–48 °C bulk-heating goal; and epidermal load is counted from **44 °C** (basal-layer injury onset, Henriques–Moritz Arrhenius; irreversible by ~50 °C). Empirical backing: ruby-laser thermal imaging found follicles reaching the highest temperature rises showed the deepest damage. The calibration (a single ~6 J/cm² dark-hair flash → ~69–72 °C at the pigmented bulb) sits squarely in the coagulation band. Sources in the simulator (18–23). Many more Ulike-family and competitor filings exist (K250194, K251984, K260742, Mlay, Philips Lumea, Alovea…) but omit pulse width entirely, so they're reachable only via **Manual**.
...
Controls: energy (**per-flash J/cm²** or **total J ÷ pulses**) · **spot size** · **gap** (hero) · **flash count** · **pulse width** (drives skin safety) · **skin tone** (melanin competition) · **hair coarseness** · **contact cooling** · single-flash compare. A built-in `SANITY()` self-test sweeps ~10,800 parameter combinations and asserts **12 physics invariants** (monotonic/bounded temps, stacking, energy-split identity, darker-skin→hotter-epidermis-but-cooler-follicle) — run it from the browser console.
