...
> 🔥 **[▶ Open the interactive SHR & Multi-Flash Thermal Simulator](shr_thermal_simulator.html)** — pick an FDA-pulse-width-verified device (or go manual), set energy per-flash **or as one total that splits across your pulse count**, and watch the epidermis, follicle, deep stem-cell target, and **sensory heat / sting proxy** heat up live. It flags whether the follicle crosses the damage threshold, whether the **skin crosses its burn threshold** (driven by fluence, pulse width, skin tone & cooling), whether sting load builds from first pulse to last, and compares a single flash to a multi-flash train. (Open `shr_thermal_simulator.html` in a browser, or via the 🔥 tool link in the IPL viewer.)
...
2. **Ulike's SHR cadence (~4 flashes/second, ~250 ms apart) is far slower than the follicle's ~10–100 ms cooling time**, so successive flashes do **not** stack much into a higher follicle *peak* — each peak mostly resets. But that does **not** mean all heat disappears: deeper tissue and local heat-pain/sting signaling cools or decays on a slower hundreds-of-ms clock, so the **last pulse can sting the most** even when the red follicle peaks are not climbing. Professional SHR works because it uses **higher fluence (5–15 J/cm²) at ~10 Hz** — enough to hold a sustained plateau; the home version is a scaled-down, comfort-first cousin.
...
- **Heat-pain / sensory load (~42–43 °C threshold, fast-rise / slow-decay proxy):** human heat-pain thresholds sit around the low-40s °C, TRPV1 is linked to noxious heat detection, and recent heat-pain imaging shows the response can rise late during a sustained heat stimulus.[[20]](https://pubmed.ncbi.nlm.nih.gov/11872608/)[[21]](https://www.nature.com/articles/s41598-025-99247-7)[[22]](https://pubmed.ncbi.nlm.nih.gov/36454632/) This is the missing piece behind your "why does the last pulse sting most?" question: subjective sting can accumulate even when follicle-peak stacking is weak.

The gap between the follicle's own cooling (~10–100 ms), the sensory/deep heat clock (hundreds of ms), and the time heat needs to be *sustained* to reach the stem cells (~170–1000 ms) is the entire physical case for multi-flash SHR. The named mechanism is **progressive photothermolysis**: *"Repeated and fast emission of pulses of low energy progressively heats the chromophore to temperatures of 45–50° over a period of time and safeguards the epidermis from overheating as opposed to a sudden rise in temperature to 65° in conventional systems."*[[10]](https://ijdvl.com/methods-to-overcome-poor-responses-and-challenges-of-laser-hair-removal-in-dark-skin/)

---

## 3. The two damage pathways, made visible

The simulator models four structures (epidermis, follicle/bulb, bulge stem cells, and a fast-rise / slow-decay sensory heat/sting proxy) as coupled thermal compartments and reports which pathway, if any, reaches threshold:
...
The failure mode sits between them: split the energy **too thin, too slow, or too few times** and *neither* follicle-damage pathway reaches threshold. Load **Ulike Air 10 — SHR** (250 ms cadence, ~1.7 J/cm²/flash): the follicle sawtooths and mostly resets between flashes, while the sensory/sting proxy can still climb from first pulse to last. Now drag the **gap dial down to ~40 ms** and watch follicle peak stacking appear — the clearest demonstration that **timing, not total energy, is the hero variable.**
...
**Temperature reality-check (spot-checked against the literature).** The simulator's thresholds hold up: **follicle coagulation ≈65 °C** matches the clinical target (65–70 °C; 60 °C minimum; time-dependent 70 °C@1 ms / 62 °C@100 ms); the **sustained-damage floor ≈45 °C** matches SHR's stated 45–48 °C bulk-heating goal; epidermal load is counted from **44 °C** (basal-layer injury onset, Henriques–Moritz Arrhenius; irreversible by ~50 °C); and the new **sting-load** threshold uses human heat-pain data around **42–43 °C**. Empirical backing: ruby-laser thermal imaging found follicles reaching the highest temperature rises showed the deepest damage. The calibration (a single ~6 J/cm² dark-hair flash → ~69–72 °C at the pigmented bulb) sits squarely in the coagulation band. Sources in the simulator (18–26). Many more Ulike-family and competitor filings exist (K250194, K251984, K260742, Mlay, Philips Lumea, Alovea…) but omit pulse width entirely, so they're reachable only via **Manual**.
...
2. **Feel the hero dial.** On the Ulike default, drag the **gap from 250 → 40 ms** — the single most important thing to see. At 250 ms, the follicle peak mostly resets while the purple sting line can build; at ~40 ms, follicle peak stacking appears because the next pulse arrives before the follicle has cooled.
...
Controls: energy (**per-flash J/cm²** or **total J ÷ pulses**) · **spot size** · **gap** (hero) · **flash count** · **pulse width** (drives skin safety) · **skin tone** (melanin competition) · **hair coarseness** · **contact cooling** · single-flash compare. A built-in `SANITY()` self-test sweeps ~1,944 representative parameter combinations and asserts **13 physics invariants** (monotonic/bounded temps, stacking, energy-split identity, darker-skin→hotter-epidermis-but-cooler-follicle, and sensory heat build-up across slow pulse trains) — run it from the browser console.
...
- **The sting model is a proxy, not a validated pain predictor.** It is anchored to heat-pain threshold, TRPV1/noxious-heat biology, and late heat-pain dynamics, but subjective pain depends on nerves, anxiety, body site, inflammation, contact pressure, and cooling. Treat it as an explanation for first-to-last pulse sensation, not as a safety clearance.
